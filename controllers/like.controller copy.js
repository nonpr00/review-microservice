...
export const getBookReviews = async (req, res) => {
  try {
    const reviews = await Review.find({ id_libro: Number(req.params.bookId) }).sort({ fecha: -1 });
    res.json(reviews);
  } catch (err) {
...
export const getUserReviews = async (req, res) => {
  try {
    const reviews = await Review.find({ id_usuario: Number(req.params.userId) }).sort({ fecha: -1 });
    res.json(reviews);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
